...
在之前的篇幅中我们主要讨论了leader的崩溃。follower和candidate的崩溃更加容易处理，二者的处理方式是一样的。如果一个follower或者candidate宕机，未来的RequestVote和AppendEntries RPC都无法得到回应。raft对此的应对方式就是无限重复呼叫。如果宕机的服务器又重新上线了，那么这些rpc最终都能得到回应。如果服务器在完成Rpc之后，返回rpc结果之前宕机，它会收到相同的rpc。Raft RPC是幂等的，所以这不会造成任何伤害。例如，如果一个追随者接收到一个包含其日志中已存在的日志条目的AppendEntries请求，它会忽略该请求中的这些条目

### 5.6 Timing and availability

对raft来说，安全性不能依赖于时间：某些事件发生的快与慢不能影响到整个系统的输出。然而，整个系统对于客户来说的可用性不可避免地依赖于时间。举例来说，如果信息交换的时间高于服务器宕机的时间，整个系统可能无法选出leader，进而无法稳步提供服务。
选举leader是raft中对时间管理最为严格的部分。只要符合以下的要求，raft就可以选出一个稳定的leader：
广播时间 远小于 选举时间 远小于 MTBF
MTBF指的是单个机器平均的崩溃时间
随机化选举超时的时间，我们可以避免“脑裂”的情况

### 7. Log Compaction
Raft服务器的日志不能无限制地增长。如果无限制增长，最终会导致过量信息影响系统可用性。系统快照是日志压缩最简单的办法，整个系统的状态被写成一个快照，针对某个日志 entry 做了快照之后，该 entry 以及以前的日志变都可以被截断（truncate）。
Figure12展示了raft日志快照的基本思路。每个服务器独立地进行快照，只覆盖本机的已经提交的日志。主要的工作就是把状态机自身的状态写入快照中。raft协议在快照中还包含了一些元数据：最后包含的索引是快照替换的日志中最后一个条目的索引（也就是状态机已经应用的最后一个条目），而最后包含的任期是这个条目的任期。在生成快照时，Raft 还会保存一些关键的元数据，用于记录快照的范围和相关信息。
最后包含的索引（last included index）记录的是被快照替换掉的日志中最后一个条目的索引位置。
最后包含的任期（last included term）记录的是与该条目相关联的任期。这对于确保系统的一致性和正确性非常重要。
这些元数据被保留是为了支持 AppendEntries的一致性检查，特别是为了检查快照之后的第一个日志条目，因为这个条目需要一个前一个日志的索引和任期。当 Raft 使用快照来替换一部分日志时，快照中的最后一个条目之前的所有日志条目都被删除。为了继续进行日志的追加操作，快照保留了最后包含的索引和任期，这样就可以进行一致性检查。这些信息对于快照之后的第一个日志条目尤为重要，因为该条目需要知道它的前一个条目的索引和任期来确保系统的一致性。一旦这个服务器完成了写快照的任务，它就删除last included index 之前的所有日志以及快照。当服务器成功创建并写入了一个快照后，之前的日志条目就变得不再需要。这是因为快照已经包含了这些日志所代表的状态。因此，服务器可以安全地删除这些日志条目，从最早的条目一直到快照中最后包含的那个条目（即 last included index）。新的快照已经包含了状态机的完整状态，因此可以删除旧的快照。这样做可以进一步节省存储空间。
尽管服务器一般独立地创建并写入快照，leader必须偶尔将自己的快照发送给follower。当follower的日志进度比leader落后很多时，可能会发生一种情况：leader已经删除了它需要发送给这个follower的某些日志条目，因为这些条目已经被包含在快照中了。此时，follower无法通过正常的日志复制过程赶上leader的进度。为了使follower同步到最新状态，leader会向follower发送一个快照。这个快照包含了状态机的当前状态和日志进度，follower可以用这个快照来更新自身的状态，并从快照后的日志条目开始继续跟随leader。
leader使用一种InstallSnapshot RPC来向落后很多的follower发送快照。当一个follower收到这个rpc, 它必须决定怎么处理本机现存的日志。一般来说，快照会包含一些这个follower所不具备的信息。这种情况下，follower抛弃它全部的日志；这些日志被快照取代，而且还有可能有没有被提交的，和leader所冲突的条目。如果follower收到的快照描述的是其日志的一个前缀（可能是由于重新传输或错误传输），那么被快照覆盖的日志条目会被删除，但快照之后的条目仍然有效，必须保留。
这个快照机制是背离了raft的强领导人机制的，因为follower可以在不知道Leader的情况下产生快照，然而，我们认为这种背离证明是可行的。虽然拥有一个领导者可以帮助避免在达成共识时出现冲突的决策，但在进行快照操作时，共识已经达成，因此不会有决策冲突。快照操作通常发生在系统已经达成共识、数据已经稳定的情况下。换句话说，所有节点在快照时刻对数据状态的理解是一致的。由于快照是在共识已经达成之后进行的，因此在快照过程中不涉及新的决策，也不会出现多个节点之间的决策冲突。快照只是保存当前一致的状态，以便在系统恢复时能够快速恢复到一致状态。数据仍然只从leader流向follower，只不过follower现在可以管理自己的数据。

### 8. Client interaction

这一节描述了客户端clerk怎样和raft端交互，包括客户端怎么样找到leader，并支持线性一致性。类似的问题在共识算法中都会被提及，raft对此的解决办法和其他系统类似。
客户端将所有的请求都发给leader。当一个客户端开机后，它首先会随机选一个服务器进行联络。如果联系到的服务器不是leader，这个服务器就会拒绝这一请求，并将最新的leader信息发给client。如果leader宕机，客户端的请求也会超时，客户端会重发消息给另一个随机选择的服务器。
我们对raft的要求是实现线性化的语义（每个操作看起来是在其调用和响应之间的某个时间点瞬时执行一次且只执行一次）。然而，根据论文之前的描述，raft可能会执行同一个命令许多次，比如说，如果leader在提交日志之后，返回给client之前宕机，客户端会换一个新Leader重试这条命令，导致这条命令被执行第二次。解决方案是为每条命令加上独特的序列号。这样，状态机追踪每个client提供的最新的命令序列号。如果状态机收到一个已经被执行过的命令，它不会执行这条命令，并返回消息。
只读操作可以在不写入日志的前提下被完成。然而，这可能会导致读到过期的数据，因为回应这个请求的leader可能已经被集群中新的Leader所取代，自己却不知道。线性化的读操作必须禁止读到过期的数据，raft需要两个预防措施来确保这件事情的发生。首先, leader必须有最新的被提交的日志的数据，leader的完备性保证leader有所有的已经提交的日志，但是在它任期刚开始的时候，它并不知道哪些是已经被提交的日志。为了验证，它必须在自己的任期内提交一个日志，因此Raft在每个任期内提交一个空命令。其次，leader在应对只读操作的时候必须检查自己是否是leader。raft对此的解决方式是发送一轮AppendEntries RPC，查看是否有大多数机器回复。